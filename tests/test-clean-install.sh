...
#!/usr/bin/env bash
[[ "$*" == "bar put oma-sleeper --before omarchy.clock" ]]
EOF
...
installed="$test_root/home/.config/omarchy/plugins/oma-sleeper"
test -x "$installed/bin/sleeper-matchup"
test -f "$installed/lib/sync-state.jq"
jq -e '.id == "oma-sleeper" and .entryPoints.barWidget == "BarWidget.qml"' "$installed/manifest.json" >/dev/null
