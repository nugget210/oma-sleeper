...
Column {
  ...
  readonly property color emphasis: colorMode === "minimal" || !hasScore ? bar.foreground : (leading ? positive : (trailing ? negative : bar.foreground))
  ...
  Row {
    width: parent.width
    Text { width: parent.width-scoreText.width; text: (root.leading && root.colorMode !== "minimal" ? "▲ " : (root.trailing && root.colorMode !== "minimal" ? "▼ " : "")) + root.teamName; elide: Text.ElideRight; color: root.emphasis; font.family: root.bar.fontFamily; font.pixelSize: Style.font.title; font.bold: true }
    Text { id: scoreText; text: Number(root.teamScore||0).toFixed(1); color: root.emphasis; font.family: root.bar.fontFamily; font.pixelSize: Style.font.title; font.bold: true }
  }
  ...
  Repeater { model: root.game ? root.game.starters : []; PlayerRow { required property var modelData; width: root.width; player: modelData; bar: root.bar; colorMode: root.colorMode } }
  ...
  Repeater { model: root.game ? root.game.bench : []; PlayerRow { required property var modelData; width: root.width; player: modelData; bar: root.bar; colorMode: root.colorMode } }
}
